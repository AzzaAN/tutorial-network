...
describe('Starting tests for angular-app', function() {
  ...
  it('navbar-brand should be angular-app',() => {
    ...
    it('RemoveHighQuantityCommodities component should be loadable',() => {
      page.navigateTo('/RemoveHighQuantityCommodities');
      browser.findElement(by.id('transactionName'))
      .then((transactionName) => {
        return transactionName.getText();
      })
      .then((txt) => {
        expect(txt).toBe('RemoveHighQuantityCommodities');
      });
    });
